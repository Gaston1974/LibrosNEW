...
func LogIn(w http.ResponseWriter, r *http.Request) (int, string, int, string) {
	...
	row := client.Cliente.QueryRow("SELECT id, last_name FROM usuarios WHERE username = ? AND clave = ?;", params.NombreUs, hashed)
	...
	msg = "Usuario " + nombre + " logueado"
	fmt.Printf("\n\n %s id: %d", msg, id)
	return 1, msg, id, nombre
}
